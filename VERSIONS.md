...
|---------|---------|------|-----------|--------------|---------------------|
| **2025.4.13** | `v2025.4.13` | 2026-05-02 | Wallpaper visible on ingress too | Themes now use `/local/cn-bg.webp` (HA Core serves on every port) instead of `/cn-bg.webp` (only nginx on 7080). `run.sh` copies the WebP into `/config/www/`. Background now appears identically on sidebar ingress (8123) and direct PWA (7080). | A customer reports the wallpaper isn't showing |
